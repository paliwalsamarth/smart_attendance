  ...
  void _showDialogPastData(List<DocumentSnapshot> list) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          title: new Text(
              "Looks like previous session was not closed properly. Click 'refresh' to start a fresh session."),
          actions: <Widget>[
...
//                Navigator.of(context).pop();
  ...
  addStudents(List studentId) async {
    globals.currentCollection =
    "attendance"; //find a method to create new collection names,
    ...
    for (int i = 0; i < studentId.length; i++) {
      ...
      Firestore.instance
          ...
          .updateData({"document": "${docRef.documentID}"});
    }
...
//                              size: 25.0, color: Color(0xff11b719)),
            ...
            StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  debugPrint(
                      "inside build but outside else if : with  class code $selectedClassCode ");
                  if (!snapshot.hasData) {
                    const Text("Loading.....");
                    debugPrint(
                        "inside if   current studentId lenght : ${globals
                            .studentId.length}");
                    return new Container();
                  } else if (globals.startAddingStudents == 1) {
                    debugPrint(
                        "inside else if: here length = ${snapshot.data.documents
                            .length} : with  class code $selectedClassCode : and documents ${snapshot
                            .data
                            .documents}   current studentId lenght : ${globals
                            .studentId.length}");
                    ...
                    for (int i = 0; i < snapshot.data.documents.length; i++) {
                      ...
                      debugPrint(
                          "current studentId lenght : ${globals.studentId
                              .length}");
                      debugPrint(
                          "current studentId var : ${globals.studentId}");
                    ...
                    debugPrint(
                        "nothing goes here : with  class code $selectedClassCode ");
                    debugPrint(
                        "current studentId lenght : ${globals.studentId
                            .length}");
                    return new Container();
...
//
   ...
   clearPastData(List<DocumentSnapshot> list) async {
    if(list.length>1) {
      await Firestore.instance.collection("attendance").document("${globals.attendance_id}").collection("attendance").document("${list[1].documentID}").delete();
      }
    else{
//      Navigator.of(context).pop();
      generationTask(); }
  ...
  getClassDetails() async {
   ...
   generationTask() async {
    QuerySnapshot querySnapshot = await Firestore.instance.collection("attendance").document("${globals.attendance_id}").collection("attendance").getDocuments();
    List<DocumentSnapshot> list = querySnapshot.documents;
    if(list.length>1) {
//      debugPrint("$list");
      _showDialogPastData(list);
    }
    else{
